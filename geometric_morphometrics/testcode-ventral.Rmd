...
setwd("./Hidden-diversity-talpid-moles_supplemnt-material")
...
# Ensure species factor levels match the order from EFA analysis
pca_data$species <- factor(pca_data$species, levels = levels(bot.p$fac$species))
...
pca_plot_procrustes <- ggplot(pca_data, aes(x = PC1, y = PC2, color = species)) +
  geom_point(size = 3) +
  # Plot individual specimens as points
  # Add semi-transparent polygons showing species clusters
  geom_polygon(
...
# Ensure species factor levels match the order from EFA analysis
df_tbl$species <- factor(df_tbl$species, levels = levels(bot.p$fac$species))
...
cva_plot_hull <- ggplot(df_tbl, aes(x = CV1, y = CV2, color = species)) +
  geom_point(size = 3) +
  # Plot specimens as colored points
  # Add semi-transparent species group polygons
  geom_polygon(
...
# Define color palette for consistent visualization (matching Euroscaptor.R)

...
library(Morpho)
##PCA
parascaptor <- read_excel("/hpcfile/home/xsn/Euroscaptor_PCA/para_log10.xlsx")
          ...
          species_names <- parascaptor[[1]]
           ...
           pca_plot <- ggplot(pca_scores, aes(x = PC1, y = PC2, color = species)) +
                    ...
                    theme_minimal() +
                    labs(title = expression("PCA of " * italic("Parascaptor") * " species - craniomandibular variables"),
...
parascaptor <- read_excel("/hpcfile/home/xsn/Euroscaptor_PCA/para_log10.xlsx")
       ...
       hull_data <- df %>%
         ...
         cva_plot <- ggplot(df, aes(x = CV1, y = CV2, color = species)) +
             ...
             scale_fill_manual(values = palette) +
             theme_minimal() +
             labs(title = expression("CVA of " * italic("Parascaptor") * " species - craniomandibular variables"),
